...
class App extends Component {
  ...
  checkImage = url => {
    ...
    if (randomImageUrl === url) {
      this.setState(prevState => ({
        ...
        randomImageUrl: imagesList[randomIndex].imageUrl,
      }))
    } else {
      clearTimeout(this.timerId)
      this.setState(prevState => ({isGameStarted: !prevState.isGameStarted}))
  ...
  render() {
    const {score, timer, activeTab, randomImageUrl, isGameStarted} = this.state
    console.log(timer)

    const tabItem = each => {
      const active = activeTab === each.tabId ? 'active-tab' : ''
      return (
        <li className="tab" key={each.tabId}>
          <button
    ...
    const renderTabsList = () => (
      <div className="container">
        <ul className="tabs-list-container">
          {tabsList.map(each => tabItem(each))}
        </ul>
      </div>
    )
    ...
    return (
      <div className="bg-container">
        <ul className="navbar">
          ...
          <li className="time-score-container">
            <p className="sm-head">
              Score:
        ...
        {isGameStarted ? (
          ...
          <ScoreCard score={score} key={score} PlayAgain={this.PlayAgain} />
        )}
      </div>
    )
  }
